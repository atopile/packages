# Microphones

A collection of microphones with digital (PMD, I2S) and analog interfaces.

## Usage

```ato
from "atopile/microphones/linkmems_lma3722t421_oa5.ato" import LinkMems_LMA3722T421_OA5_driver

module LinkMems_LMA3722T421_OA5_example:
    """
    Minimal example of how to use the LinkMems LMA3722T421-OA5 analog microphone.
    """
    microphone = new LinkMems_LMA3722T421_OA5_driver

    power_3v3 = new ElectricPower
    power_3v3 ~ microphone.power

    analog = new ElectricSignal
    analog ~ microphone.analog
```

```ato
from "atopile/microphones/linkmems_ldm2718t261_oa1.ato" import LinkMems_LMD2718T261_OA1_driver


module LinkMems_LMD2718T261_OA1_example:
    """
    Minimal example of how to use the LinkMems LMD2718T261-OA1 PDM microphone.
    """
    microphone = new LinkMems_LMD2718T261_OA1_driver

    power_3v3 = new ElectricPower
    power_3v3 ~ microphone.power

    pdm = new PDM
    pdm ~ microphone.pdm
    microphone.pdm.select.line ~ power_3v3.lv
```

```ato
from "atopile/microphones/tdk_invensense_ics_43434.ato" import TDK_InvenSense_ICS_43434_driver

module TDK_InvenSense_ICS_43434_example:
    """
    Minimal example of how to use the TDK InvenSense ICS-43434 I2S microphone.
    """
    microphone = new TDK_InvenSense_ICS_43434_driver

    power_3v3 = new ElectricPower
    power_3v3 ~ microphone.power

    i2s = new I2S
    i2s ~ microphone.i2s
```

```ato
from "atopile/microphones/knowles_sph0641lu4h_1.ato" import Knowles_SPH0641LU4H_1_driver

module Knowles_SPH0641LU4H_1_example:
    """
    Minimal example of how to use the Knowles SPH0641LU4H I2S microphone.
    """
    microphone = new Knowles_SPH0641LU4H_1_driver

    power_3v3 = new ElectricPower
    power_3v3 ~ microphone.power

    i2s = new I2S
    i2s ~ microphone.i2s
```

## Contributing

Contributions to this package are welcome via pull requests on the GitHub repository.

## License

This atopile package is provided under the [MIT License](https://opensource.org/license/mit/).
