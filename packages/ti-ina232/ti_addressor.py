...
def test_ti_addressor_parameters():
    ...
    assert (
        int(app.addressor.get().base.get().force_extract_superset().get_single()) == 0x48
    )
    assert (
...
def test_ti_addressor_sets_address_line(offset: int, expected_dest: str):
    ...
    # Run post-instantiation-setup checks (triggers address line setting)
    check_design(app, stage=F.implements_design_check.CheckStage.POST_INSTANTIATION_SETUP)
