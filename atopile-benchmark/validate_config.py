...
def validate_config(config_file: Path) -> bool:
    ...
    if "atopile_versions" in config:
        ...
        if not isinstance(versions, list):
            ...
            for i, version in enumerate(versions):
                ...
                elif version["type"] not in ["release", "branch", "commit", "local"]:
                    errors.append(
                        f"atopile_versions[{i}] has invalid type: {version['type']}"
                    )

                if "version" not in version:
                    errors.append(f"atopile_versions[{i}] missing 'version'")

                # 'date' is used for display only; allow omitting it.
    ...
    if "package_benchmarks" in config:
        ...
        if not isinstance(packages, list):
            ...
            for i, pkg in enumerate(packages):
                ...
                # 'source' is optional; many configs only specify name/package/enabled.
                required_pkg_keys = ["name", "package"]
    ...
    # Optional: local packages mode (skip `ato add`)
    skip_ato_add = bool(config.get("skip_ato_add", False))
    if skip_ato_add and not config.get("local_packages_root"):
        errors.append(
            "skip_ato_add is enabled but local_packages_root is not set (path to packages checkout)"
        )
    ...
    print(
        f"  Packages: {len([p for p in config.get('package_benchmarks', []) if p.get('enabled', True)])}"
    )
    print(f"  Build Commands: {len(config.get('build_commands', []))}")
    print(
        f"  Total Benchmarks: {len(config.get('atopile_versions', [])) * len([p for p in config.get('package_benchmarks', []) if p.get('enabled', True)]) * len(config.get('build_commands', []))}"
    )
